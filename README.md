...
- **Limiter styles** — per profile: `hard` (GT-style bounce: timed fuel cut, sharp audio chop, exhaust pop per cut; 458, LS3, 370Z, AE86), `soft` (torque taper holds RPM at redline while the engine keeps singing, with a gentle hunt; DeLorean, 454, Astra), `none`, or legacy hysteresis. Straight-cut gear whine and the limiter loop are kept for the track car only; road cars get a quiet helical gear hum.
