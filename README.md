...
- **Coach** — names each mistake the moment it happens (LOCK-UP, TOO FAST IN, POWER UNDERSTEER, LIFT-OFF / POWER OVERSTEER, WHEELSPIN) with a one-line why; rate-limited so it never nags.
- **Records that remember you** — PB lap, best sectors (IDEAL lap), per-corner min-speed records ("CORNER RECORD +3 km/h"), speed-trap record, PB brake points; saved per car and shown on the car-select cards. Chimes for PBs, purple sectors and corner records.
